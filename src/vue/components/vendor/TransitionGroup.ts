...
const TransitionGroupImpl: ComponentOptions = {
  ...
  setup(props: TransitionGroupProps, { slots }: SetupContext) {
    ...
    onUpdated(() => {
      ...
      const movedChildren = prevChildren.filter(c => applyTranslation(c, props.globalScale ?? 1))
...
function applyTranslation(c: VNode, globalScale: number): VNode | undefined {
  ...
  if (dx || dy) {
    ...
    s.transform = s.webkitTransform = `translate(${dx / globalScale}px,${dy / globalScale}px)`
